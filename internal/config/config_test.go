...
func TestIsOSCEnabled_AutoSSH(t *testing.T) {
	cfg := DefaultConfig()
	cfg.Notifications.OSC.Enabled = nil
	t.Setenv("SSH_CONNECTION", "1.2.3.4 5678 5.6.7.8 22")
	if !cfg.IsOSCEnabled() {
		t.Fatal("expected true in SSH auto mode")
	}
}

func TestIsOSCEnabled_AutoNoSSH(t *testing.T) {
	cfg := DefaultConfig()
	cfg.Notifications.OSC.Enabled = nil
	...
	t.Setenv("SSH_TTY", "")
	if cfg.IsOSCEnabled() {
		t.Fatal("expected false outside SSH in auto mode")
	}
}

func TestIsOSCEnabled_ExplicitTrue(t *testing.T) {
	cfg := DefaultConfig()
	enabled := true
	...
	t.Setenv("SSH_TTY", "")
	if !cfg.IsOSCEnabled() {
		t.Fatal("expected true with explicit enabled=true even without SSH")
	}
}

func TestIsOSCEnabled_ExplicitFalse(t *testing.T) {
	cfg := DefaultConfig()
	enabled := false
	cfg.Notifications.OSC.Enabled = &enabled
	t.Setenv("SSH_CONNECTION", "1.2.3.4 5678 5.6.7.8 22")
	if cfg.IsOSCEnabled() {
		t.Fatal("expected false with explicit enabled=false even with SSH")
	}
}

func TestIsAnyNotificationEnabled_IncludesOSC(t *testing.T) {
	cfg := DefaultConfig()
	cfg.Notifications.Desktop.Enabled = false
	...
	if !cfg.IsAnyNotificationEnabled() {
		t.Fatal("expected true when only OSC is enabled")
	}
}

func TestValidateOSCFormat_Invalid(t *testing.T) {
	cfg := DefaultConfig()
	cfg.Notifications.OSC.Format = "invalid_format"
	if err := cfg.Validate(); err == nil {
		t.Fatal("expected validation error for invalid OSC format")
	}
}

func TestValidateOSCFormat_Valid(t *testing.T) {
	for _, format := range []string{"", "auto", "osc777", "osc9", "osc99"} {
		t.Run(format, func(t *testing.T) {
			cfg := DefaultConfig()
			cfg.Notifications.OSC.Format = format
			if err := cfg.Validate(); err != nil {
				t.Fatalf("unexpected validation error for format %q: %v", format, err)
			}
		})
	}
}

func TestIsStatusOSCEnabled(t *testing.T) {
	cfg := DefaultConfig()
	enabled := true
	cfg.Notifications.OSC.Enabled = &enabled

	if !cfg.IsStatusOSCEnabled("task_complete") {
		t.Fatal("expected IsStatusOSCEnabled true for known enabled status")
	}

	// Disable a specific status
	disabled := false
	cfg.Statuses["question"] = StatusInfo{Enabled: &disabled, Title: "Q"}
	if cfg.IsStatusOSCEnabled("question") {
		t.Fatal("expected IsStatusOSCEnabled false for disabled status")
	}
}
